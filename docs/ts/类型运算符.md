...
这个例子`keyof MyObj`返回`MyObj`的所有键名组成的联合类型，即`'foo'|'bar'`
...
这个列子中，`string & keyof Obj`等同于`string & string|number|symbol`进行交集运算，最后返回`string`，因此`Capital<T extends string>`就不会报错了。
...
在js中in用来确定对象是否包含某个属性名，在ts 类型运算中，`in`运算符用来取出（遍历）联合类型的每一个成员类型。
